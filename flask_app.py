...
def make_pdf(data, output_path):
    ...
    # 右側ヘッダーは box を使わず線のみで構成
    # 外枠の横線（上）y=5mm
    c.line((L+87)*mm, H-5*mm, R*mm, H-5*mm)
    # 外枠の横線（下）y=35mm ← 機種名行の上線と一致させる
    c.line((L+87)*mm, H-35*mm, R*mm, H-35*mm)

    # 縦線：会社名と承認の境界 x=L+149
    c.line((L+149)*mm, H-5*mm, (L+149)*mm, H-35*mm)
    # 縦線：承認と担当の境界 x=L+172
    c.line((L+172)*mm, H-5*mm, (L+172)*mm, H-35*mm)
    # 縦線：右端 x=R
    c.line(R*mm, H-5*mm, R*mm, H-35*mm)

    # 承認・担当のラベル横線 y=12mm
    c.line((L+149)*mm, H-12*mm, R*mm, H-12*mm)
